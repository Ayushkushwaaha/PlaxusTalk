...
function useScreenShare(localVideoRef, localStreamRef, pcRef) {
  ...
  const start = useCallback(async () => {
    try {
      ...
      if (pcRef.current) { const sender = pcRef.current.getSenders().find(s => s.track?.kind === 'video'); if (sender) await sender.replaceTrack(track); }
...
function ReactionsBar({ roomId, socket }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      {floating.map(f => <div key={f.id} style={{ position: 'fixed', bottom: '140px', left: `${f.x}%`, fontSize: '36px', pointerEvents: 'none', zIndex: 9999, animation: 'floatUp 2.5s ease forwards' }}>{f.emoji}</div>)}
      <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', flexWrap: 'wrap' }}>
        {EMOJIS.map(e => <button key={e} onClick={() => sendReaction(e)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '22px', padding: '2px 4px', transition: 'transform 0.15s' }} onMouseOver={ev => ev.currentTarget.style.transform='scale(1.35)'} onMouseOut={ev => ev.currentTarget.style.transform='scale(1)'}>{e}</button>)}
      </div>
    </div>
  );
}

// FIX: Defined OUTSIDE RoomPage — prevents remounting on every render
function RemoteVideoTile({ videoRef, isConnected, peerCount }) {
  return (
    <div style={{ position: 'relative', borderRadius: '14px', overflow: 'hidden', background: '#0a0a10', border: '1px solid rgba(255,255,255,0.07)', height: '100%', minHeight: '160px' }}>
...
function LocalVideoTile({ videoRef, isVideoOff, isSharing, userName }) {
  return (
    <div style={{ position: 'relative', borderRadius: '14px', overflow: 'hidden', background: '#0a0a10', border: '1px solid rgba(0,255,136,0.15)', height: '100%', minHeight: '160px' }}>
      <video ref={videoRef} autoPlay playsInline muted style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block', transform: 'scaleX(-1)', visibility: isVideoOff ? 'hidden' : 'visible' }} />
      {isVideoOff && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
          <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: 'rgba(0,255,136,0.12)', border: '2px solid rgba(0,255,136,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', color: '#00ff88', fontSize: '24px', fontWeight: 700 }}>{userName?.charAt(0)?.toUpperCase() || 'A'}</div>
...
function Btn({ onClick, active, danger, title, children, wide, activeColor }) {
  ...
  return (
    <button onClick={onClick} title={title} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ width: wide?'52px':'44px', height:'44px', borderRadius: danger?'14px':'50%', border:'none', cursor:'pointer', fontSize:'18px', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0,
        background: danger?'linear-gradient(135deg,#ef4444,#dc2626)':active?(activeColor?activeColor.replace('0.55','0.2').replace('0.6','0.2'):'rgba(255,107,53,0.2)'):h?'rgba(255,255,255,0.15)':'rgba(255,255,255,0.08)',
        boxShadow: danger?'0 4px 14px rgba(239,68,68,0.4)':active?`0 0 0 2px ${ac}`:'none', transition:'all 0.2s', transform:h?'scale(1.06)':'scale(1)' }}>
      {children}
    </button>
  );
}
...
function MobileChatDrawer({ open, onClose, roomId, user, socket }) {
  ...
  return (
    <>
      ...
      <div style={{ position:'fixed', bottom:0, left:0, right:0, height:expanded?'85vh':'50vh', zIndex:300, background:'rgba(10,10,16,0.98)', borderTop:'1px solid rgba(255,255,255,0.1)', borderRadius:'20px 20px 0 0', display:'flex', flexDirection:'column', transition:'height 0.3s cubic-bezier(0.4,0,0.2,1)', boxShadow:'0 -8px 32px rgba(0,0,0,0.5)' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px 8px', flexShrink:0, position:'relative' }}>
          <div style={{ width:'40px', height:'4px', borderRadius:'2px', background:'rgba(255,255,255,0.2)', position:'absolute', left:'50%', transform:'translateX(-50%)', top:'8px' }} />
          <span style={{ fontFamily:'monospace', fontSize:'11px', color:'#4a4a5c', letterSpacing:'0.2em' }}>💬 CHAT</span>
          <div style={{ display:'flex', gap:'8px' }}>
            <button onClick={() => setExpanded(e => !e)} style={{ background:'none', border:'none', color:'#4a4a5c', cursor:'pointer', fontSize:'14px' }}>{expanded?'▼':'▲'}</button>
        ...
        <div style={{ position:'relative' }} ref={menuRef}>
          ...
          {menuOpen && (
            <div style={{ position:'absolute', bottom:'54px', right:0, width:'205px', background:'#0d0d12', border:'1px solid #1a1a24', borderRadius:'14px', overflow:'hidden', zIndex:200, boxShadow:'0 8px 28px rgba(0,0,0,0.7)' }}>
              <div style={{ padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}><span style={{ fontFamily:'monospace', fontSize:'9px', color:'#4a4a5c', letterSpacing:'0.2em' }}>MORE OPTIONS</span></div>
              {[
    ...
    return (
      <div style={{ height:'100vh', display:'flex', flexDirection:'column', background:'#060608', overflow:'hidden', color:'#e8e8f0' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'6px', padding:'8px', flexShrink:0, height:'220px' }}>
          <RemoteVideoTile videoRef={setRemoteVideoRef} isConnected={isConnected} peerCount={peerCount} />
          <LocalVideoTile videoRef={setLocalVideoRef} isVideoOff={isVideoOff} isSharing={isSharing} userName={user?.name} />
        </div>
        <div style={{ padding:'4px 8px', flexShrink:0 }}><ReactionsBar roomId={roomId} socket={socket} /></div>
        <div style={{ padding:'6px 8px', flexShrink:0 }}><ControlBar /></div>
        ...
        <style>{`@keyframes floatUp{0%{opacity:1;transform:translateY(0) scale(1)}100%{opacity:0;transform:translateY(-150px) scale(1.4)}} @keyframes blink{0%,100%{opacity:1}50%{opacity:0.3}}`}</style>
      </div>
    );
  }
    ...
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', background:'#060608', overflow:'hidden', color:'#e8e8f0' }}>
      ...
      <div style={{ flex:1, display:'flex', overflow:'hidden', minHeight:0 }}>
        <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'12px', gap:'10px', minHeight:0 }}>
          ...
          <div style={{ flexShrink:0 }}><StatsPanel latency={latency} isP2P={isP2P} connectionState={connectionState} iceState={iceState} peerCount={peerCount} callId={callId} /></div>
        </div>
        <div style={{ width:'275px', borderLeft:'1px solid rgba(255,255,255,0.07)', background:'rgba(10,10,16,0.7)', display:'flex', flexDirection:'column', overflow:'hidden', flexShrink:0 }}>
          <div style={{ padding:'10px 14px', borderBottom:'1px solid rgba(255,255,255,0.07)' }}><span style={{ fontFamily:'monospace', fontSize:'10px', color:'#4a4a5c', letterSpacing:'0.2em' }}>SESSION PANEL</span></div>
          <div style={{ display:'flex', borderBottom:'1px solid rgba(255,255,255,0.07)', flexShrink:0 }}>
            {['chat','info'].map(t => (
              <button key={t} onClick={() => setSideTab(t)} style={{ flex:1, padding:'9px', background:sideTab===t?'rgba(0,255,136,0.06)':'none', border:'none', borderBottom:sideTab===t?'2px solid #00ff88':'2px solid transparent', color:sideTab===t?'#00ff88':'#4a4a5c', fontFamily:'monospace', fontSize:'10px', cursor:'pointer', letterSpacing:'0.12em', textTransform:'uppercase', transition:'all 0.2s' }}>
                {t==='chat'?'💬 Chat':'⚙ Info'}
              </button>
            ))}
          </div>
      ...
      <style>{`@keyframes blink{0%,100%{opacity:1}50%{opacity:0.3}} @keyframes floatUp{0%{opacity:1;transform:translateY(0) scale(1)}100%{opacity:0;transform:translateY(-150px) scale(1.4)}}`}</style>
    </div>
  );
}
